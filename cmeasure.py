...
class ContextMeasure:
    ...
    def compute(self, fm: np.ndarray, gt: np.ndarray, img: np.ndarray = None) -> float:
        ...
        """
        X = self._preprocess_maps(fm, binary_flag = False)
        Y = self._preprocess_maps(gt, binary_flag = True)
    ...
    def _calculate_cac(self, X: np.ndarray, Y: np.ndarray, kernel: np.ndarray) -> tuple:
        X = X.astype(float)
        ...
        cac = math.e/ (math.e-1) * Y * global_completeness_matrix
        return cac

    def _preprocess_maps(self, img, binary_flag) -> np.ndarray:
        """
        Preprocesses the input maps: converts to grayscale, normalizes fm, and binarizes gt.

        Parameters:
            fm (numpy.ndarray): Foreground map (grayscale).
            gt (numpy.ndarray): Ground truth map (grayscale).

        Returns:
            tuple: (normalized grayscale fm, binarized gt)
        """
    ...
    def _calculate_camouflage_degree(self,
        img: np.ndarray,
        mask: np.ndarray,
        w: int = 7
    ) -> tuple:
    ...
    def _ann_with_spatial(self,
        x: np.ndarray, q: np.ndarray,
        x_coords: np.ndarray, q_coords: np.ndarray,
        m: int = 16
    ) -> np.ndarray:
        """
